...
[![codecov](https://codecov.io/gh/wickra-lib/wickra-feature-store/branch/main/graph/badge.svg)](https://codecov.io/gh/wickra-lib/wickra-feature-store)
[![Go module](https://raw.githubusercontent.com/wickra-lib/.github/main/profile/badges/go.svg)](https://pkg.go.dev/github.com/wickra-lib/wickra-feature-store-go)
[![License: MIT OR Apache-2.0](https://img.shields.io/badge/license-MIT_OR_Apache--2.0-blue)](https://github.com/wickra-lib/wickra-feature-store#license)

**Streaming feature matrices for Go, over the Wickra C ABI hub via cgo.**

Wickra Feature Store turns a spec JSON into a deterministic feature/label matrix —
indicator, price and microstructure columns folded once in a Rust core, so the
result is byte-identical across every language and between the parallel and
sequential build paths. This package is the Go binding; it consumes the C ABI hub
through cgo and drives the store over the same JSON protocol as every other
binding.

## Install

Use the published **`wickra-feature-store-go`** module, which bundles the prebuilt
...
```go
import wickra "github.com/wickra-lib/wickra-feature-store-go"
```

`wickra-feature-store-go` is generated from the [`bindings/go`](https://github.com/wickra-lib/wickra-feature-store/tree/main/bindings/go)
directory by the release pipeline: it mirrors the Go sources, the vendored C ABI
header (`include/wickra_feature_store.h`) and the prebuilt libraries under
...
The `bindings/go` directory in the [wickra-feature-store](https://github.com/wickra-lib/wickra-feature-store)
...
cp target/release/libwickra_feature_store.so    lib/linux_amd64/    # Linux
cp target/release/libwickra_feature_store.dylib lib/darwin_arm64/   # macOS (arm64)
cp target/release/wickra_feature_store.dll      lib/windows_amd64/  # Windows
```
...
- **Runnable examples:** [`examples/go/`](https://github.com/wickra-lib/wickra-feature-store/tree/main/examples/go)
...
Wickra Feature Store is analytics software, not a trading system. The values it
computes are deterministic transforms of the input data — they are not financial
